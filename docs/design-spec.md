# 建築法令リファレンス（BLRA）実装設計書 v1.3

- 作成日: 2026-07-29（2026-08-02 v1.3 改訂）
...
| v1.2 | 2026-07-30 | 表示変換と原文座標の分離（§6.2）、Citation Resolver の欠陥3件を Normative 化（§6.3）、レンダリング戦略と性能予算（§19.22）、本文書体を明朝へ訂正。ADR-027〜029 |
| v1.3 | 2026-08-02 | hourei-rag を稼働ベースとする Phase 1 Reader Profile を正本化。選択法令の全文一括取得・全文DOM、目次／横断検索中心の2カラム構成、現行版のみの閲覧方針を追加（§1.5） |
...
| 6 | North Star Metric を Evidence Set 数から「一級建築士法規の未見課題における根拠条文セット到達率」＋「Evidence Set 第三者再現成功率」へ変更 | 外部参加者の確保や所要時間計測に依存せず、毎年同じ形式で回帰評価できる | §14.4・ADR-031 |
...
### 1.5 Phase 1 Reader Profile（2026-08-02 Normative Amendment）

方針転換後の実装は `web/` の hourei-rag（Next.js 14 + Prisma + PostgreSQL）を稼働ベースとする。本節は Phase 1 の実装範囲について本書の他節に優先する。旧 `src/` の Fastify + Kysely 実装は独立資産として保持するが、Phase 1 画面の実行経路には含めない。

Phase 1 の目的は、建築実務者が収録済み法令の現行本文へ速く到達し、目次・検索・条文間参照を使って読み進められる法令リーダーを成立させることである。案件管理、根拠保存、注釈、チェック項目、AI 推薦、専門家レビューは将来フェーズへ延期する。

#### 1.5.1 本文の取得・表示

- 選択中の 1 法令について、収録済み現行 `LawRevision` の全ノードを 1 回の API 応答で取得する。
- クライアントは取得した全ノードを 1 つの連続した本文 DOM として描画する。章単位の追加取得、無限スクロール、仮想化を Phase 1 の既定経路にしない。
- 全文表示でも、Prisma DB に保存した階層、構造化番号、本文、表、インライン表記および確定済み内部リンクを正本とし、既存の条文レンダリング規則を再利用する。
- 各条・附則・別表の固定アンカーは DB の Article ID から決定論的に生成する。目次操作は同一ページ内をスクロールし、URL を置き換える。ページ再読込後も同じ条文へ復元できること。
- 初期表示で対象条文を先に見せるため、アンカー移動は全文描画完了直後に行う。追加の条文取得を待たせない。

#### 1.5.2 画面構成と遷移

- デスクトップは左ナビゲーションと本文の 2 カラムを基本とし、常設の右パネルは置かない。左ナビゲーションは「目次」と「検索」の 2 タブとする。
- モバイルでは左ナビゲーションをドロワー化する。
- 目次からの移動は同一タブ内の本文スクロールとする。検索結果および条文間参照の移動先は、比較を保つため新しいブラウザタブで開く。
- 法令本文に存在しない実務上の関連は本文へ推測で埋め込まない。公式の条文見出し（`ArticleCaption`）だけを使う「論点索引」は別機能として段階追加する。

#### 1.5.3 データと信頼表示

- Phase 1 は収録済み現行版だけを閲覧対象とし、旧版本文を保存・提供することを要件にしない。
- 案件別の適用時点バー、経過措置の自動通知、6状態設計は Phase 1 から外す。画面には収録データの基準日と e-Gov の公式法令ページへの導線を表示する。
- 本文内リンクとして利用者に提示するのは、DB に保存され、対象条文が存在する解決済み参照だけとする。未解決候補や機械推定結果は本文リンクとして公開しない。
- 改正履歴・施行状況の詳細は独自統合せず、e-Gov の公式画面へ案内する。

#### 1.5.4 画面状態と性能受入条件

画面状態は通常、空、読込中、部分失敗、全面失敗の 5 状態を区別する。全文 API は同一 `LawRevision` のノードだけを順序保証して返し、公開本文の構築中に案件・注釈・推薦・履歴・チェック項目の API を呼び出さない。性能の受入測定は、代表法令と収録中最大法令について、初期表示、深い条文へのアンカー移動、本文内検索およびスクロール応答を対象とする。
...
未施行版（`valid_from > D`）は検索結果に含めず、一般ユーザー画面には表示しない。詳細な施行予定は e-Gov 公式ページへ誘導する（現行法令増分更新設計 §13.3 参照）。
...
- アンカーを切り替えたとき、表示中の条文の内容が変わる場合は差分を提示する。黙って内容を差し替えない。現行法令の更新時には、変更通知バナー（現行法令増分更新設計 §13.2）により変更された条番号を示し、詳細は e-Gov 公式ページへ誘導する。
...
**v1.2 で新設。** 稼働中の類似実装（`docs/reference/hourei-rag-extraction.md`）から取り込んだ論点である。

#### 6.2.1 なぜ必要か

紙の法令集の見た目に寄せる場合、本文を**表示用に変換して**描画することになる。市販の法令集は横書き化に伴い、漢数字を算用数字へ、単位名称を記号へ、割合を分数へ変換している。
...
この変換を行うと、**表示テキストの文字オフセットが原文とずれる。** その結果、注釈・ハイライト・引用範囲が原文へ紐づかなくなる。§6.1 の `text_quote_selector` は「表示テキスト＝原文」を暗黙に前提しており、変換を入れた瞬間に破綻する。

#### 6.2.2 モデル

原文を表示トークン列へ変換し、各トークンが原文座標を保持する。

```text
DisplayToken {
  sourceStart: int      原文上の開始オフセット
  sourceEnd:   int      原文上の終了オフセット
  displayText: string   画面に出る文字列
  kind:        PLAIN | NUMBER | UNIT | FRACTION
}
```

- `PLAIN` は表示＝原文であり、オフセットを直接換算できる
- `NUMBER` / `UNIT` / `FRACTION` は変換済みであり、**一部を選択しても範囲をトークン全体へ拡張する**（「4m」の「4」だけを選択させない）

#### 6.2.3 原文座標を DOM に持たせない

**この座標対応表は、データとして保持し、DOM 属性として出力しない。**

トークンごとに `<span data-source-start=...>` を出力する実装は、座標問題を解決する代わりに DOM ノード数を桁で増やし、描画性能を破壊する。実測では、本文 33KB の章に対しペイロード 685KB・表示 2.3 秒という結果が観測されている（§19.22）。

利用者の選択範囲から原文座標を求める処理は、選択発生時に座標対応表を二分探索すればよい。DOM に持たせる必要はない。

#### 6.2.4 保存する座標は常に原文座標とする

Annotation、SavedItem、EvidenceSnapshot が保持する範囲は、**すべて原文座標**とする。表示変換の規則が将来変わっても、保存済みの根拠が壊れないためである。表示座標を保存してはならない。
...
#### 6.4.1 必須要件（v1.2 追加・F-6 の実測に基づく）

次の3点は実データで欠陥を確認済みであり、実装の必須要件とする。

**(a) 号の枝番を切り捨てない。** 「第四号の二」は `item4` ではない。第四号と第四号の二は別の規定である。号の枝番（`号の二`）と、その下のイロハ階層に対応する。

**パースできない枝番を検出した場合、切り捨てた path を返してはならない。** `unresolved` として §7.3 の未解決参照へ回す。誤った条項を confidence 付きで返す silent truncation は、未解決より悪い。設計書 §18 に対する直接の違反である。

**(b) 法令略称の辞書を運用データとして持つ。** 「建基法」「都計法」等は実務で日常的に使われる。市販法令集が略称一覧として28件を定義していることが、その裏付けである。コードに埋め込まず、運用データとして更新可能にする。

**(c) 号は漢数字を第一表記とする。** 法令の慣行では、条・項を算用数字にしても号は漢数字のまま書く（「8条1項四号の二」）。検索クエリの解釈では両方を受け付け、**表示では漢数字**とする。
...
**検索用の正規化と、表示用の変換（§6.2）を混同しない。** 前者は索引と照合のため、後者は可読性のためであり、規則が異なる。表示変換では次を原文のまま維持する。

- 区分・等級（「第一種」「一級」）
- 号（法令の慣行。§6.4.1-c）
- 公布番号（「昭和二十五年法律第二百一号」）

この除外規則は、市販法令集の表記方法と一致している（`docs/reference/hourei-shu-2026-ocr.md` §2.1）。
...
- 一級建築士「学科Ⅲ（法規）」の学習セット120問について、全選択肢から派生した論点別検索課題
- 未見評価30問に対する論点別の期待 Provision・複数の到達経路・例外・未収録資料と、
  問題全体の統合判断の正解セット
- Recall@10 / MRR の自動計測
- CI での回帰検出
...
未見評価は30問のため、正解セット1〜2件の誤りで結果が反転しうる。公式正答肢と適用法令日を保持し、根拠条文セットは実装者が確認する。独立検査者を確保できる場合はゲート時にサンプリング確認するが、確保を開発の前提にしない。

各問題は「問題→全選択肢由来の派生論点→統合判断」の階層で扱う。正答肢の主要論点だけを
登録した問題は部分データであり、問題単位の評価対象にしない。全選択肢を派生論点でカバーし、
全論点と統合判断を試験年度の法令で確認した場合だけ検証済みとする。計算問題等で複数の選択肢が
同じ判断過程を共有する場合は、一つの論点で複数の選択肢番号をカバーしてよい。詳細は
`docs/superpowers/specs/2026-08-09-compound-architect-law-problem-design.md` に従う。
...
3. 引用された抜粋が Source **原文**と文字列一致する（表示変換後のテキストと照合してはならない。§6.2）
...
**描画予算（v1.2 追加）。** 上記の応答時間目標は、サーバー応答だけでなく利用者が読める状態になるまでを指す。したがって次を予算として定める。根拠と背景は §19.22。

| 項目 | 予算 |
|---|---|
| 章表示のペイロード | 本文テキスト量の **3 倍以内** |
| 章表示時の DOM ノード数 | **5,000 以内** |
| 本文トークンあたりの DOM ノード | **0**（トークン単位の要素を生成しない） |

いずれも自動計測し、超過を CI で検出する。
...
> 一級建築士法規の未見課題における、根拠条文セット到達率

令和7年30問を要件・辞書の学習に使わず、必要な法・令・規則・告示、法令間経路、例外、
未収録資料を過不足なく確認できた割合で測る。検索単体は Recall@10 80%以上を目標とし、
根拠条文セット到達率はカテゴリ別に報告する。

**準主指標**

> EvidenceSnapshot の第三者再現成功率（目標 90% 以上）

Evidence Set 数そのものは生成頻度に左右されるため、North Star にはしない。カウンタとしては保持する。
...
Phase 0' Architect Law Exam Learning（Phase 0 と並行可）
...
Phase 6  Exam Benchmark & Decision Gate
```
...
### 15.3 Phase 0': 一級建築士法規過去問による要件学習（並行）

| # | 内容 | 合格条件 |
|---|---|---|
| U-1 | 過去問の課題分類 | 一級建築士「学科Ⅲ（法規）」令和3〜6年120問について、全選択肢から案件条件・派生論点・対象法令・複数の根拠経路を付与 |
| U-2 | 未見ベンチマーク | 令和7年30問を学習に使わず、論点別の検索・根拠条文・複数経路・例外・未収録表示と問題全体の統合判断を評価 |
| U-3 | 検索課題への変換 | 問題文・選択肢を転載せず、全選択肢をカバーする派生論点と根拠条文セットを作成 |
| U-4 | 年次更新 | 新年度問題を未見評価へ追加し、前年を学習セットへ移動 |

**Exit Criteria**: 学習120問と未見評価30問について全選択肢由来の派生論点を固定し、評価対象は
全論点と統合判断を検証済みとする。主要検索課題で Recall@10 80%以上を達成すること。外部参加者の
人数・組織数は Exit Criteria にしない。
...
Exit: Researcher → Reviewer の End-to-End フローが成立。T-04 が通る。評価用成果物を Export できる。

**Phase 5: AI Assistance & Hardening**

AI Gateway、Context Builder、Citation Validator、検索語支援、Evidence 要約、脅威モデリング、Backup/Restore 訓練、監査レビュー、評価実行手順、フィードバック収集。

Exit: Security Critical Issue が解消。Backup/Restore 試験成功。T-05・T-07 が通る。AI Citation 付与率 100%。

**Phase 6: Exam Benchmark & Decision Gate**

一級建築士法規の未見評価30問を本システムで実施し、派生論点ごとの検索順位、根拠条文セット、
複数の法令間経路、例外、未収録表示、誤根拠と、問題全体の統合判断を計測する。

判断: `GO`（拡大） / `ITERATE`（改善して再評価） / `PIVOT`（対象 JTBD またはプロダクト形態の変更） / `STOP`。

### 15.5 過去問ベンチマーク成功条件

1. 令和3〜6年120問について、全選択肢由来の派生論点と統合判断のメタデータが完成
2. 令和7年30問を学習から隔離し、全選択肢の論点カバー、公式正答肢、論点別根拠条文セットを固定
3. 主要検索課題の Recall@10 が 80% 以上
4. 基準日時点の Provision 表示正確性が 99% 以上
5. 論点別・問題別の根拠条文セット到達率をカテゴリ別に計測し、重大な根拠欠落が 0 件
6. 例外・未収録資料を黙って省略するケースが 0 件
7. EvidenceSnapshot の第三者再現成功率 90% 以上
8. 重大な Authority / Version 誤表示が 0 件
9. AI を有効にする場合、AI 出力の Citation 付与率 100%
10. 評価対象問題の選択肢由来論点カバー率が 100%
11. 複合問題完了率と統合判断正答率を学習・未見評価に分けて報告
...
**Solo Track では S5（過去問ベンチマーク）完了まで着手しない。** 並行トラックは本線と可処分時間を奪い合うため、1 名体制では本線の遅延要因にしかならない。Rule Candidate は学習120問と未見評価30問で観測された根拠経路から抽出する。
...
| 評価コーパス | 外部参加者による Pilot | **学習120問＋未見評価30問** |
| 検索課題 | 50 件 | 30 件 |
...
S4  Hardening                                 3週
S5  Exam Benchmark                            4週
                                     合計    32週
```

フルタイム換算で約 7〜8 か月である。兼業であればこの 1.5〜2 倍を見込む。この数字を短く見積もらないこと。工期超過の主因は実装ではなく、コーパスの品質確認と例外対応に費やす時間である。

**S0（3週）** — §15.2 の F-1〜F-9 を実施する。§15.3 の U-1〜U-4 は、一級建築士「学科Ⅲ（法規）」の過去問分析として S0 と並行して行う。外部参加者の確保を S1 の前提にしない。**S0 の Exit Criteria は緩めない。** ここで告示の入手可否が確定しなければ、S1 へ進まない。
...
**S5（4週）** — 未見評価30問のベンチマーク、回帰修正、Go/No-Go 判断。実務者フィードバックは任意で併用するが、人数・組織数を合格条件にしない。
...
| S5 | 未見評価の Recall@10 が 60% 未満、または重大な根拠欠落が解消できない |

### 15.10 Solo Track における評価条件

§15.5 をそのまま適用する。外部参加者の人数・組織数は条件に追加しない。

学習120問・未見評価30問という規模は縮小しない。正確性に関する条件（時点表示正確性99%、
Snapshot再現成功率90%、重大な Authority / Version 誤表示0件）も緩和しない。
...
| O-4 | 対象自治体の確定 | 過去問分析後 | 未収録・地域依存課題の棚卸しを踏まえて手動登録範囲を決める |
...
| O-9 | 独立検査者の人選 | ゲート前 | 確保できる場合のみサンプリング検査を依頼。未確保でも S1 を止めない（ADR-025） |
...
**ADR-016**（本書で新規） — 実装体制を Solo Track（実装 1 名＋AI 支援）とし、フェーズを §15.9 のとおり再構成する。ただし建築法令 Domain Reviewer を外部に有償で確保することを必須条件とし、実装者による兼任を禁じる。表示する条文の正確性は、実装者が AI を用いても自己検証できないためである。Domain Reviewer 未確保のまま S1 へ進まない。**→ 一部改訂（ADR-025）: 実装者が二級建築士・建築実務経験者であることが確認されたため、Domain Reviewer の継続的確保は S1 の前提から外した。独立チェックはゲート時のサンプリング検査へ縮小。Solo Track 採用と正確性基準の不変は維持する。**
...
**ADR-018**（本書で新規、ADR-031 で評価方法を改訂） — Solo Track では外部参加者数を成功条件にせず、学習120問・未見評価30問で評価する。正確性に関する成功条件（時点表示正確性99%、Snapshot再現成功率90%、重大な Authority / Version 誤表示0件）は緩和しない。
...
**ADR-027**（v1.2 新規） — 表示変換（漢数字→算用数字等）を行う場合、Citation Anchor と保存済みの範囲は**すべて原文座標**で保持する。表示座標を保存しない。座標対応表はデータとして持ち、DOM 属性として出力しない。Citation Validator の本文一致検証は表示テキストではなく原文に対して行う。

**ADR-028**（v1.2 新規） — 本文トークンごとに DOM 要素を生成しない。ハイライトは CSS Custom Highlight API を用い、DOM を変更せずに描画する。章全体の DOM を一度に materialize せず仮想化する。認証内側の法令本文を SSR しない。§14.1 の描画予算（章の DOM 5,000ノード以内、ペイロードは本文の3倍以内）を CI で計測する。

**ADR-029**（v1.2 新規） — コーパスのクライアント一括配信を推奨構成とする。MVP 対象3法令の本文は計 670 KB（gzip 概算 150 KB）であり、現状の1章分のペイロードより小さい。**ただし採用はプロファイル結果を待つ。** 2.3秒の内訳がサーバー応答側かレンダリング側かで、着手すべき対策の順序が変わる。測定より先に実装しない。

**ADR-030**（v1.2 新規） — 実装言語を変更しない。ADR-022（TypeScript / Node 22）を維持する。単一条文が 0.08 秒、章が 2.3 秒という実測は、言語ではなくペイロードと DOM の量に起因する。バックエンド言語の変更は応答時間の 300ms 部分にしか効かず、2 秒側には効かない。
...
7. **読み込みの速さは体験の一部である** — 法令集は開いた瞬間に読める。待ち時間は本設計における機能欠陥として扱う。実装制約は §19.22 に定める。
...
- **法令本文の書体は明朝（Hiragino Mincho ProN / Yu Mincho / Noto Serif JP 系）を標準とする。** v1.1 ではゴシック標準としていたが、実物照合として誤りであった。市販法令集の本文は明朝であり、稼働中の類似実装も明朝を採用している。UI ラベル・見出し・バッジはゴシックとし、本文と役割を分ける
...
**v1.2 で新設。** 稼働中の類似実装で観測された実測値に基づく。
...
一方、実コーパスの本文量は次である。建築基準法の全条文で 211 KB、最大の章でも **33 KB** にすぎない。

**本文 33 KB を表示するのに 685 KB を送り 2.3 秒かかっている。** 単一条文が 0.08 秒で出ている以上、これは言語や実行環境の速度ではない。**ペイロードと DOM の量に起因する。**

#### 19.22.2 禁止事項

1. **本文トークンごとに DOM 要素を生成しない。** 原文座標の保持は §6.2.3 のとおりデータ側で行う
2. **章全体の DOM を一度に materialize しない。** 可視範囲外の条文は DOM から外す（仮想化）
3. **認証の内側にある法令本文を SSR しない。** SEO 上の利益がなく、hydration の費用だけが残る
4. **本文以外のメタデータを本文と同一ペイロードで送らない。** ハイライト、リンク、実務論点、レコメンドは遅延取得する

#### 19.22.3 ハイライトの実装

線引き・注釈のハイライトは、**CSS Custom Highlight API** を用いる。

```js
CSS.highlights.set("user-highlight", new Highlight(range1, range2, ...));
```

Range オブジェクトで任意の範囲を着色でき、**DOM を一切変更しない。** 従来の span 包囲方式が抱える DOM 増加と、ハイライト追加ごとの再描画を同時に解消する。

利用可能ブラウザは Chrome 105+ / Safari 17.2+ / Firefox 140+ である。業務用デスクトップを主対象とする本サービス（§19.4.1）では前提として差し支えない。非対応環境では、ハイライトを無効化したうえで本文を表示する（機能縮退であり、本文が読めなくなってはならない）。
...
| 建築基準法施行令 | 214 KB |
| 建築基準法施行規則 | 245 KB |
| **合計** | **670 KB**（gzip 概算 150 KB） |

**MVP コーパス全体が、現状の1章分のペイロード（685 KB）より小さい。**

したがって次を推奨構成とする。

> 公開済み Provision の本文と構造を、**版ごとの不変スナップショットとしてクライアントへ一括配信し、以後は端末側で保持する。**

一度取得すれば、条文表示・章スクロール・参照ジャンプ・章内検索がネットワーク往復なしで完結する。法令データは公開後に不変であり（ADR-004）、更新頻度も低いため、キャッシュ戦略が単純に成立する。

サーバーへ残すのは、案件データ、注釈、Evidence、権限、AI、全文検索（コーパス拡大時）である。

**採用の判断はプロファイル結果を待つ。** 現状の 2.3 秒がサーバー応答側に偏っているなら本方式が直接効き、hydration 側に偏っているなら §19.22.2 の禁止事項が先に効く。**測定より先にこの構成を実装しない。**

#### 19.22.5 計測の義務

次を CI で自動計測し、§14.1 の描画予算に対する回帰を検出する。

- 章表示時の DOM ノード数
- 章表示時のペイロード総量と、うち本文テキストの比率
- サーバー応答 / 転送 / レンダリングの内訳

**「速い気がする」で判断しない。** 予算超過はバグとして扱う。
