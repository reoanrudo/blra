...
再取込後の現行DBに対して §2.1 の前提数値を再計測した。スコープは
`currentLawBookArticleScopeSql` 相当（ksk-2026・現行Revision・soft delete 除外）で再現。

### 15.1 計測値の更新

| 項目 | 2026-08-02（設計時） | 2026-08-07（現行） |
|---|---:|---:|
| スコープ内 Article | — | 121,535 |
| 公式見出しを持つ条文 | 17,126 | 10,142 |
| 正規化見出しの種類 | 8,745 | 6,995 |
| 見出しを持つ法令 | 120 | 114 |

**判定: ライブProjectionの採用判断（§3.1）は現データでも成立する。** データ量は設計時より
小さく、ライブ集約の負荷はさらに低い。

### 15.2 前提となっていた事例の無効化

- 「施行期日」（設計時 112法令・2,515条文）は現行スコープで **0件**。附則条文の caption が
  現行取込では保存されていないことが原因と見られる。§13.4 の E2E 例は「目的」（現行
  52条文・51法令）で引き続き成立するため、E2E は「目的」を正の事例として使う。
- 最大の論点は「定義」55条文（51法令）。50件を超える論点が現存するためページング
  （§6.4）の要件は維持するが、大規模ページングの性能リスクは設計時より小さい。
- `ja-x-icu` 照合は現行 PostgreSQL に存在することを確認（§6.2 のデプロイ前提は成立）。

### 15.3 実務用語のカバーに関する校正

公式見出しは構造的見出し（目的・定義・準用・権限の委任・経過措置・欠格条項等）には
強いが、実務用語の入口としては機能しない。現行スコープでの実測: 排煙 0件、
内装制限 0件、耐火構造 0件、防火区画 2件、避難階段 1件。

**帰結**: 論点索引は「公式の構造見出しによる横断索引」として要件を限定する。
「排煙」等の実務用語からの到達は本機能の責務とせず、検索基盤強化
（`2026-08-07-related-navigation-requirements.md` §6.8）と将来のタグ拡張（§11）で扱う。
同要件文書の US-3 にはこの校正を反映済み。

### 15.4 副次的発見（委任参照との接続）

建築基準法に「国土交通省令への委任」という公式見出しを持つ条文が2件存在する。
論点索引が委任参照 Stage A（法令単位の導線）の入口の1つとして自然に機能する。
