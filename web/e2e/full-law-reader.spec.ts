...
test.describe("Phase 1 全文法令リーダー", () => {
  ...
  test("法令本文の上部に施行日・確認状態を表示しない", async ({ page }) => {
    await page.goto(`/articles/${TEST_ARTICLE_ID}`);
    await expect(page.locator('[data-full-law-ready="true"]')).toBeVisible();

    await expect(page.getByText(/e-Gov.*版/)).toHaveCount(0);
    await expect(page.getByText(/施行日:/)).toHaveCount(0);
    await expect(page.getByText(/e-Gov更新:/)).toHaveCount(0);
    await expect(page.getByText(/最終確認:/)).toHaveCount(0);
    await expect(
      page.getByText(/e-Govとの最新確認が完了していません。/),
    ).toHaveCount(0);
  });
